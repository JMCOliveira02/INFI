...
class Scheduling():
    ...
    def __calculateEdgesWeights(self, edges):
        '''
        ...
        Args:
            edges: lista de arestas obtidas das transformações simples. Deste modo evita-se calcular
    ...
    def schedule(self, recipe: Recipe, status: str, is_even: bool = True):
        ...
        if status in {"waiting", "stashed"}:
            ...
            nodes, edges = findSimpleTransformations(self.G_simple, recipe.piece_out)
            if len(nodes) == 0 or len(edges) == 0:
                return -2

        '''
        CALCULAR COM BASE NO is_even !!!!!!!
        '''
        
        # é possível realizar a transformação. Atualiza pesos das arestas.
        all_machines_busy = self.__calculateEdgesWeights(edges)
        ...
        recipe.finished_date = recipe.sended_date + datetime.timedelta(seconds=recipe.time/1000)
